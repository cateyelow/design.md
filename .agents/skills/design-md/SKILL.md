...
Write the brief in a few lines: who arrives, what they doubt, the one action, and what they must understand in the first two screens. Collect real material: photos (a phone photo is fine), prices, hours, addresses, product screens, reviews with their source. Mark what is missing in the brief; the page leaves it out without announcing the gap. Do not invent numbers, testimonials, client logos or people; a section without material is removed, not filled. Concrete material is the strongest defence against a generic page.
...
Then cut the copy. Every time, price, date, phone number and rule has one home, where a reader looking for that kind of fact goes (every price in the price list when there is one); an action may carry what it needs and the footer may hold the contact details once. When two devices show the same data, remove one whole. No sentence announces what a table shows, restates a heading or tells the reader what the page covers or leaves out, and the first screen is what and where, at most two short sentences and the action. Read the page as plain text at both widths, delete every second statement, then repair fragments, headings and gaps the cut left; see "Copy: each fact once" in [references/craft.md](references/craft.md).
...
- The copy checks in `audit.py` count times, prices, dates and phone numbers per block (a table, a list or a run of same-shaped items is one block; links, buttons, the footer, fixed banners and screen-reader-only text are left out) and compare sentences across blocks. They flagged all three agent-built evaluation pages (6, 6 and 3 repeated facts, plus restating blocks, paraphrased sentences and sentences about the page) and nothing on the 43 human-made sites, mostly brand and service homepages, they were tuned against. Small local-business sites were missing from that set, and copy that repeats an idea in new words is not detected. Fresh agents running only the step 7 cut pass on those three pages removed 35 to 56 percent of the visible text with every brief fact kept. The cuts that damaged a page in that round (a timeline stripped of its times, a heading left over two unrelated rows, an emptied table column) became the repair list in craft.md, and a second round on the two damaged pages with that list removed whole devices instead (50 and 42 percent less text, no copy warnings).
